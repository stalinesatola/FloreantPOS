...
public class MenuItem extends BaseMenuItem {
	...
	public TicketItem convertToTicketItem(OrderType orderType, double itemQuantity) {
		...
		List<Tax> taxList = getMenuItemTaxes();
		if (this.getMenuItemTaxes() != null){
			for (Tax tax : taxList){
				TicketItemTax ticketItemTax = convertToTicketItemTax(tax, ticketItem);
				ticketItem.addToTaxes(ticketItemTax);
			}
		}
	...
	public static TicketItemTax convertToTicketItemTax(Tax tax, TicketItem ticketItem) {
		TicketItemTax ticketItemTax = new TicketItemTax();
		java.lang.Integer itemCount = ticketItem.getItemCount();
		java.lang.Double itemUnitPrice = ticketItem.getUnitPrice();
		java.lang.Double taxAmount = itemUnitPrice*tax.getRate()/100*itemCount;
		ticketItemTax.setAmount(taxAmount);
		ticketItemTax.setName(tax.getName());
		ticketItemTax.setRate(tax.getRate());
		ticketItemTax.setTax(tax);
		ticketItemTax.setTicketItem(ticketItem);
	...
	//retrieves a list of all taxes for and item
	public List<Tax> getMenuItemTaxes() {
		return getTaxGroup().getTaxes();
	}
